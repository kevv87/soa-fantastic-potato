...
class OpenAiIface {
  async _ask_openai(platilloPrincipal, bebida, postre) {
    ...
    try {
      ...
      throw new OpenAIError(`OpenAI failed to do something: ${error.message}`, error.status);
    }
  }
  ...
  async getRecomendacion(platilloPrincipal, bebida, postre, res) {
    try {
      ...
      if (error instanceof OpenAIError) {
        console.log('OpenAPI couldnt send a response ');
...
class DatabaseIface {
  ...
  _getPredefinedResponse(category) {
    ...
    if (!responses) throw new CategoryNotFoundError(`Not found category: ${category}`);
    if (responses.length == 0) throw new RecomendacionNotFoundError(`There are no recomendations for ${category}`);
  ...
  async getRecomendacion(platilloPrincipal, bebida, postre, res) {
    ...
    try {
      ...
      if (error instanceof CategoryNotFoundError) {
        console.log("Category not found, telling requester we couldn't " + ' find the resource');
        res.status(error.httpsCode);
        res.json({ message: error.message });
        console.error(error.message);
